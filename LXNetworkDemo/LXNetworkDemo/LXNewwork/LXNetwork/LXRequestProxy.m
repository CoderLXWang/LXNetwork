...
+ (instancetype)sharedInstance
{
    ...
    static LXRequestProxy *sharedInstance = nil;
    dispatch_once(&onceToken, ^{
        sharedInstance = [[LXRequestProxy alloc] init];
    });
...
//    params = [NSDictionary sjParamsForGETComplementWithParams:params];
    ...
    self.sessionManager.requestSerializer.timeoutInterval = kLXNCacheOverdueSeconds;
...
- (NSNumber *)callApiWithParams:(NSDictionary *)params url:(NSString *)url methodName:(NSString *)methodName requestType:(RequestType)type success:(LXProxyCallback)success fail:(LXProxyCallback)fail upload:(multipart)upload
{
    ...
    suceesBlock sblk = ^(NSURLSessionDataTask *task, id reponseObject) {
        ...
        LXResponse *response = [[LXResponse alloc] initWithRequestId:requestId request:task.originalRequest params:params reponseObject:reponseObject];
    ...
    failureBlock fblk =  ^(NSURLSessionDataTask *task, NSError *error) {
        ...
        LXResponse *response = [[LXResponse alloc] initWithRequestId:requestId request:task.originalRequest params:params error:error];
